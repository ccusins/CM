...
async function setBookmakerToDone(bookmakerHolder, makeVisible) {
    ...
    let bookieTitle = bookmakerHolder.querySelector('.bookmaker_title');
    let ogTitle = bookieTitle.textContent;
    ...
    const bookmakerResponse = await fetch(`/cmbettingapi/getbookmakerdetails/${userid}`);

    const responseData = await bookmakerResponse.json();
    let bookmakerData = responseData.data.data;

    bookmakerData.forEach(bookie => {
        if (bookie.bookmaker == ogTitle) {
            let newBookieTitle = `${ogTitle}\n${bookie.bookmakerUsername}\n${bookie.bookmakerEmail}\n${bookie.bookmakerPassword}`;
            bookieTitle.textContent = newBookieTitle.replace(/\n/g, '<br>')
            bookieTitle.innerHTML = bookieTitle.textContent;
            return;
        }
    });
...
async function setUpSubMenu(index) {
    ...
    for (let i = 1; i < index+1; i++) {
        ...
        subMenuDiv.addEventListener('click', () => {
            let containerDisplay = document.querySelector(`#stage-${i}-container`);
            containerDisplay.style.display = 'flex';
            containerDisplay.style.display.flexDirection = 'column';
            let containers = document.querySelectorAll('.acc_container');
            containers.forEach(container => {
                if (container !== containerDisplay) {
